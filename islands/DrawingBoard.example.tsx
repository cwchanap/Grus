...
export default function GameRoomExample({ roomId, playerId, playerName }: GameRoomExampleProps) {
  const [gameState, setGameState] = useState<GameState>({
    ...
    players: [
      ...
      { id: playerId, name: playerName, isHost: false, isConnected: true, lastActivity: Date.now() }
    ],
    ...
    <div class="game-room-example max-w-6xl mx-auto p-4">
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-lg shadow-md p-4 mb-4">
            ...
            <div class="space-y-2 text-sm">
              ...
              <div><strong>Round:</strong> {gameState.roundNumber}</div>
              <div><strong>Phase:</strong> {gameState.phase}</div>
              <div><strong>Time:</strong> {timeRemaining}s</div>
              {gameState.phase === 'drawing' && (
                <div><strong>Word:</strong> {gameState.currentDrawer === playerId ? gameState.currentWord : '???'}</div>
              )}
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-md p-4 mb-4">
            ...
            <div class="space-y-2">
              {gameState.players.map(player => (
                <div key={player.id} class={`flex items-center justify-between p-2 rounded ${
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <span class={`text-sm ${player.id === gameState.currentDrawer ? 'font-semibold' : ''}`}>
                      {player.name}
      ...
      <div class="mt-6 bg-white rounded-lg shadow-md p-4">
        ...
        <div class="flex gap-2">
          <input
            type="text"
            placeholder={gameState.currentDrawer === playerId ? "You're drawing!" : "Type your guess..."}
      ...
      <div class="mt-6 bg-blue-50 rounded-lg p-4">
        ...
        <div class="text-sm text-gray-700 space-y-1">
          <p><strong>When it's your turn to draw:</strong> Use the drawing tools to illustrate the given word. Others will try to guess!</p>
          <p><strong>When others are drawing:</strong> Watch the drawing and type your guesses in the chat. Faster correct guesses earn more points!</p>
          <p><strong>Game phases:</strong> Drawing → Guessing → Results → Next Turn</p>
        </div>
      </div>
    </div>
  );
