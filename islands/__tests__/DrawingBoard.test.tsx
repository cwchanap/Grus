...
function createMockGameState(overrides: Partial<GameState> = {}): GameState {
  return {
    ...
    phase: 'drawing',
    players: [
      { id: 'player1', name: 'Player 1', isHost: true, isConnected: true, lastActivity: Date.now() },
      { id: 'player2', name: 'Player 2', isHost: false, isConnected: true, lastActivity: Date.now() }
    ],
...
Deno.test("DrawingBoard - WebSocket message handling", () => {
  ...
  const mockHandleDrawingUpdate = (data: { command?: DrawingCommand; commands?: DrawingCommand[] }) => {
    if (data.command) {
...
Deno.test("DrawingBoard - Drawing permissions based on game state", () => {
  ...
  const checkDrawingPermissions = (playerId: string, gameState: GameState) => {
    return gameState.currentDrawer === playerId && gameState.phase === 'drawing';
  };
...
Deno.test("DrawingBoard - Message validation and error handling", () => {
  ...
  const handleMessage = (data: string) => {
    try {
      ...
      invalidMessages.push({ error: error instanceof Error ? error.message : 'Unknown error', data });
    }
  };
