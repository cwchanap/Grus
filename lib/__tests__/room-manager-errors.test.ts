...
class MockDBService extends KVRoomService {
  ...
  override updateRoom(
    ...
    >,
  ): Promise<{ success: boolean; data?: boolean | undefined; error?: string | undefined }> {
...
withTestRoomManager(
  "RoomManager - createRoom handles player creation failure",
  async (roomManager, db) => {
...
withTestRoomManager(
  "RoomManager - createRoom handles room update failure",
  async (roomManager, db) => {
