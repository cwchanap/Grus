...
export class RoomManager {
  ...
  async createRoom(params: CreateRoomParams): Promise<CreateRoomResult> {
    try {
      const { name, hostName, gameType, maxPlayers = 8, isPrivate = false } = params;
      ...
      const roomResult = await this.db.createRoom(name.trim(), null, maxPlayers, gameType, isPrivate);
