...
export class RoomManager {
  ...
  createRoom(params: CreateRoomParams): CreateRoomResult {
    try {
      ...
      const roomResult = this.db.createRoom(name.trim(), null, maxPlayers, gameType);
      ...
      // Create host player
      const playerResult = this.db.createPlayer(
        hostName.trim(),
        roomId,
        true,
      );
      ...
      const updateResult = this.db.updateRoom(roomId, { hostId: actualPlayerId });
      if (!updateResult.success) {
        // Clean up if update fails
        this.db.removePlayer(actualPlayerId);
        this.db.deleteRoom(roomId);
        return { success: false, error: "Failed to update room with host" };
      }

      // Get the created room with players
      const roomSummary = this.getRoomSummary(roomId);
  ...
  joinRoom(params: JoinRoomParams): JoinRoomResult {
    try {
      ...
      const updatedRoomSummary = this.getRoomSummary(roomId);
    ...
    try {
      ...
      if (wasHost && players.length > 1) {
        ...
        if (newHost) {
          ...
          const updateResult = this.db.updatePlayer(newHostId, { isHost: true });
          if (!updateResult.success) {
      ...
      const removeResult = this.db.removePlayer(playerId);
      ...
      if (players.length === 1) { // Only the leaving player
        const deleteResult = this.db.deleteRoom(roomId);
        if (deleteResult.success) {
          ...
          this.db.updateRoom(roomId, { isActive: false });
        }
      }
  ...
  getRoom(roomId: string): Room | null {
    try {
      const result = this.db.getRoomById(roomId);
  ...
  getRoomSummary(
    roomId: string,
  ): { success: boolean; data?: RoomSummary; error?: string } {
    try {
      const roomResult = this.db.getRoomById(roomId);
      if (!roomResult.success || !roomResult.data) {
        return { success: false, error: "Room not found" };
      }

      const playersResult = this.db.getPlayersByRoom(roomId);
  ...
  cleanupEmptyRooms(
    ...
    try {
      ...
      for (const room of rooms) {
        const playersResult = this.db.getPlayersByRoom(room.id);
        if (playersResult.success) {
          const players = playersResult.data || [];
   ...
   */
  getActiveRoomsWithCleanup(
    limit = 20,
  ): { success: boolean; data?: RoomSummary[]; error?: string } {
    try {
      // First, clean up empty rooms
      this.cleanupEmptyRooms(limit * 2); // Check more rooms than we need to return
      ...
      for (const room of rooms) {
        const summaryResult = this.getRoomSummary(room.id);
