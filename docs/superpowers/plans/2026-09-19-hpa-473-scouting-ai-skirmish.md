...
- Missing `VisibilityMap` means full information for focused pure-sim tests/benchmark only. Normal runtime setup/restart always inserts it.
...
- [ ] Add `VISION_RADIUS_CELLS = 8` as the one initial radius. Do not add per-unit vision tables in this ticket.
...
- [ ] Implement `refresh_visibility(world, map)`: living units + completed buildings reveal; incomplete sites do not.
...
- [ ] Add helpers used later: `cell_state(team, cell)`, `is_visible(team, cell)`, `is_explored(team, cell)`, plus target/source footprint helpers as narrowly needed.
...
- Modify: `crates/grus-sim/src/fixture.rs` only for test helpers if needed
...
- [ ] Add standalone-resource knowledge check to Gather when visibility exists: unexplored source rejects, explored source remains gatherable after current vision is lost. Own Farm remains known; enemy Farm follows enemy-building visibility.
- [ ] Insert initial visibility setup after normal skirmish seed/reset.
...
- [ ] Add `sync_view_visibility` beside metadata/health Update systems using the existing `GodotNodeHandle`.
...
- [ ] Gate combat cosmetics: never spawn a tracer from a hidden enemy origin. Own-visible damage/death feedback may still show at the visible target.
...
- [ ] Add focused Godot smoke coverage for initial hidden enemies, reveal, hide, stale selection clearing, no hidden minimap marker and minimap recenter.
- [ ] Keep HPA-470/HPA-471/HPA-472 smokes green; do not copy their full journeys into the new smoke.
...
- [ ] AI may query all own state. Filter standalone resources by explored state before considering them.
...
- [ ] Required invariance regression: clone identical own/observed AI state, move hidden enemies to different unseen cells, and assert the next AI command list is identical.
...
- [ ] Gate AI decisions with the existing Playing check. Start/Paused/Result must produce no AI commands.
...
- [ ] Add a bounded pure-Rust full-match journey with ordinary systems/commands: economy -> scouting -> army -> combat -> Result. No exact tick arithmetic.
...
- [ ] Add one runtime HPA-473 smoke that incrementally verifies fog/minimap integration, ordinary AI economic progress, scouting/visible combat, Result and Restart freshness.
