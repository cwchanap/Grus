...
- [ ] Implement each ordered policy step as a pure decision function over `&World`, controller/decision state and `&AiMapPlan`, returning zero/one `PlannedCommand` — a `PlayerCommand` plus an optional deferred controller transition. `decide_ai_commands()` is their ordered composition; `step_ai()` is cadence + apply + commit. Acceptance-sensitive controller progress (scout-route cursor, army-kind cursor) commits only when the command's `CommandResult` accepts it, so a rejected command cannot consume progress.
