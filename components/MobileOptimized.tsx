...
export function TouchButton({ 
  ...
  const baseClasses = "font-medium rounded-lg transition-all duration-200 touch-manipulation no-tap-highlight no-select";
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      class={`${baseClasses} ${variantClasses[variant]} ${sizeClasses[size]} ${activeClasses} ${className}`}
    >
      {children}
    </button>
  );
...
export function TouchInput({
  ...
  const baseClasses = "border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors touch-manipulation";
