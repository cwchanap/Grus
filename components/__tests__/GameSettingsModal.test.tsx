...
Deno.test("GameSettingsModal - calculateTotalTime works correctly", () => {
  ...
  testCases.forEach(({ settings, expected }) => {
    ...
    assertEquals(result, expected, `Expected ${expected} seconds for ${JSON.stringify(settings)}, got ${result}`);
  });
});
...
Deno.test("GameSettingsModal - formatTimeDisplay formats correctly", () => {
  ...
  testCases.forEach(({ settings, expected }) => {
    ...
    assertEquals(result, expected, `Expected "${expected}" for ${JSON.stringify(settings)}, got "${result}"`);
  });
});

Deno.test("GameSettingsModal - estimateGameTime calculates reasonable estimates", () => {
  const testCases = [
    { settings: { maxRounds: 5, roundTimeMinutes: 1, roundTimeSeconds: 30 }, expectedMin: 11, expectedMax: 13 },
    { settings: { maxRounds: 3, roundTimeMinutes: 2, roundTimeSeconds: 0 }, expectedMin: 8, expectedMax: 10 },
    { settings: { maxRounds: 10, roundTimeMinutes: 1, roundTimeSeconds: 0 }, expectedMin: 14, expectedMax: 16 }
  ];

  testCases.forEach(({ settings, expectedMin, expectedMax }) => {
    const result = estimateGameTime(settings);
    assertEquals(result >= expectedMin && result <= expectedMax, true, 
      `Expected estimate between ${expectedMin}-${expectedMax} minutes for ${JSON.stringify(settings)}, got ${result}`);
  });
});
